      ...
      return this.each(function () {
        ...
        priceBut.onclick = () => {
          if (cartPricesLoaded[0]) {
            ...
            let fenceBoardsCart180180 = 0;
            let fenceBoardsCart90180 = 0;
            let fenceBoardsCartKosi = 0;
            let fenceBoardsCart18090 = 0;
            let fenceBoardsCartKapija = 0;
            for (let i = 0; i < fenceBoards.length; i++) {
              if (fenceBoards[i][0].isVisible && fenceBoards[i][1].isVisible)
                fenceBoardsCart180180 += 1;
              if (fenceBoards[i][0].isVisible && !fenceBoards[i][1].isVisible)
                fenceBoardsCart18090 += 1;
              if (fenceBoards[i][2].isVisible) fenceBoardsCart90180 += 1;
              if (fenceBoards[i][4].isVisible || fenceBoards[i][5].isVisible)
                fenceBoardsCartKosi += 1;
              if (fenceBoards[i][6].isVisible) fenceBoardsCartKapija += 1;
            }
            //populate posts
            ...
            let post235SilberCart2set = 0;
            let post190SilberCart2set = 0;

            if (allPosts[0].material.diffuseColor.r > 0.3) {
              if (allPosts[0].scaling.z > 1.4) {
                post295SilberCart += 1;
              } else if (
                allPosts[0].scaling.z > 1 &&
                allPosts[0].scaling.z < 1.4
              ) {
                post235SilberCart += 1;
              } else if (allPosts[0].scaling.z < 1.1) {
                post190SilberCart += 1;
              }
            } else {
              if (allPosts[0].scaling.z > 1.4) {
            ...
            spojevi.forEach((elm) => {
              elm.forEach((element) => {
                if (element.isVisible) spojeviCart += 1;
              });
            });
            spojeviCart = Math.ceil(spojeviCart / 6);
            //populate sturmanker
            ...
            cartItems = [];
            cartItems.push(
              fenceBoardsCart180180,
              fenceBoardsCart90180,
              fenceBoardsCartKosi,
              fenceBoardsCart18090,
              fenceBoardsCartKapija,
              post295SilberCart,
              post235SilberCart,
              post190SilberCart,
              post150SilberCart,
              ...
              post190AnthCart,
              post150AnthCart,
              post235SilberCart2set,
              post190SilberCart2set,
              rootsCart,
              spojeviCart,
